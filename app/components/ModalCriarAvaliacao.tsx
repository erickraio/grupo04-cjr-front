    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#ebebeb] dark:bg-[#2A2A2A] w-full max-w-[650px] rounded-[2.5rem] p-10 flex flex-col items-center gap-6 relative shadow-2xl transition-colors duration-300">
        ...
        <h2 className="text-2xl font-normal text-black dark:text-gray-200 mt-2 transition-colors duration-300">
          Você está avaliando <span className="font-semibold text-black dark:text-white transition-colors duration-300">{nomeProduto}</span>
        </h2>

        <div className="flex gap-2 my-2">
          {[1, 2, 3, 4, 5].map((estrela) => (
            <button key={estrela} onClick={() => setCriarNota(estrela)} className="transition-transform hover:scale-110 w-14 h-14 cursor-pointer">
        ...
        <div className="w-full bg-white dark:bg-[#1A1A1A] rounded-2xl p-6 min-h-[220px] shadow-inner flex flex-col transition-colors duration-300">
          <textarea
            ...
            className="w-full flex-1 bg-transparent outline-none resize-none text-gray-700 dark:text-white text-base font-light placeholder-gray-400 dark:placeholder-gray-500 transition-colors duration-300"
          />
        </div>

        <div className="w-full flex flex-col gap-4 mt-4 px-12">
          <button onClick={handleCriarAvaliacao} className="w-full py-3 bg-[#6A38F3] hover:bg-[#5b2bd1] text-white font-semibold rounded-full shadow-lg text-sm tracking-wider uppercase hover:opacity-90 transition-all cursor-pointer">
            Avaliar
          </button>
        </div>
      </div>
